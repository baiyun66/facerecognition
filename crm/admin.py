...
class StudentAdmin(admin.ModelAdmin):
    list_display = ('id','qq','name','date','user','sex')
...
class CourseRecordAdmin(admin.ModelAdmin):
    list_display = ['from_class','day_num','has_sign','date']
    ...
    def initialize_studyrecords(self, request, queryset):
        ...
        print(queryset[0].from_class.StudentTOClass_set.all())
        new_obj_list = []
        for enroll_obj in queryset[0].from_class.StudentTOClass_set.all():
            new_obj_list.append(models.SignRecord(
                student = enroll_obj,
                course_record = queryset[0] ,
                attendance = 0 ,
            ))
        try:
            models.SignRecord.objects.bulk_create(new_obj_list) #批量创建
        except Exception as e:
            return HttpResponse("批量初始化学习记录失败，请检查该节课是否已经有对应的学习记录")
        return redirect("/admin/crm/signrecord/?course_record__id__exact=%s" % queryset[0].id)
    initialize_studyrecords.short_description = "初始化本节所有学员的上课记录"
    actions = ['initialize_signrecords',]
# Now register the new UserAdmin...


class SignRecordAdmin(admin.ModelAdmin):
    list_display = ['student','course_record','attendance','date']
    list_filter = ['course_record','attendance','course_record__from_class','student']
...
admin.site.register(models.StudentTOClass)
admin.site.register(models.ClassList)
admin.site.register(models.Role)
admin.site.register(models.SignRecord,SignRecordAdmin)
